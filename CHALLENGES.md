...
## Challenge 5: JS-backreference noise and Wayback timeouts aren't one-off issues
I tested the pipeline on CNN (edition.cnn.com) as a second real domain, beyond just NYT. The /subscribe page redirected to /subscription and had real, clearly legit pricing in the raw HTML - things like "$1.99/month," "$69.99," "$29.99," sitting right next to text like "subscription-card-grouped-products__pricing-info-price" and "automatically renews monthly." So unlike NYT's live site, CNN's subscribe page does expose pricing without needing JavaScript to render it.

But the same page also had the exact same kind of false positive I found on NYT - bare "$1" matches sitting inside the site's bundled JavaScript, used as a regex replacement placeholder (e.g. `e.replace(h,'<a href="$1">$1</a>')`), not an actual price. (A quick explanation of this for anyone reading: in regex, when you search for a pattern and part of it is wrapped in parentheses, you can "capture" that part and reuse it later - the capture is referred to as $1, $2, etc. depending on which group it was. So `e.replace(h,'<a href="$1">$1</a>')` doesn't mean "one dollar" at all - it means "take whatever was captured by the first group in my search pattern, and reuse it twice inside this replacement string." It just happens to look exactly like a price when my own regex, which is searching for a totally different pattern, scans over that same text.) Since I've now seen this same pattern on two unrelated sites, I don't think it's a NYT-specific quirk - it's a predictable side effect of any site that ships JavaScript using regex capture groups, which is extremely common. My price regex will basically always pick up a few of these on any real site, so context-checking each match stays necessary no matter which domain I'm checking.

I also hit repeated read timeouts from Wayback Machine while testing more domains, even after raising my timeout from 15 to 60 seconds. This happened independent of which domain or query I was running, so I don't think it's something I can fix on my end with a longer timeout alone - it seems like Wayback's servers are just inconsistently slow or unresponsive. For a full-scale version of this project, this would need proper retry logic (try again a few times with increasing wait times) rather than a single request per snapshot.
